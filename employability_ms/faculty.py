...
def faculty_dashboard():
    ...
    return render_template("Faculty/facultyEnd.html", auth_user=auth_user, students_record=students_record, unapprove_account=unapprove_account, count_unapprove=count_unapprove)
...
def signup_Superadmin():
    try:
        new_user = User(request.form['first_name'], request.form['middle_name'], request.form['last_name'], request.form['contact_number'], request.form['email'], 'faculty',  'faculty', (generate_password_hash(request.form['password'], method="sha256")), True, 0)
